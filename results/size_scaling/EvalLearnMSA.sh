...
#SBATCH --output=./learnMSA_eval.%A_%a.log
#SBATCH -J learnMSA_eval                                                                                                                                    
...
FILES=(data/subset_alignments/*.fasta)
...
projection="data/subset_alignments_proj/${FILENAME}"
...
mkdir -p tmp_learn_MSA
         
echo ${FILENAME%.fasta} $sp $modeler $tc $col >> tmp_learn_MSA/${SLURM_ARRAY_TASK_ID}.out
